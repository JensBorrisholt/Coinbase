...
namespace Coinbase
{
    ...
    public class CoinbaseApi
    {
        ...
        private static readonly Lazy<CoinbaseApi> InternalInstance = new Lazy<CoinbaseApi>(() => new CoinbaseApi(ConfigurationManager.AppSettings["ApiKey"], ConfigurationManager.AppSettings["ApiSecret"]));
        public static CoinbaseApi Instance => InternalInstance.Value;
        ...
        public CoinbaseApi(string apiKey, string apiSecret) : this(apiKey, apiSecret, CoinbaseConstants.LiveApiUrl, null)
        {
            ...
            : this(apiKey, apiSecret, null, null, useTimeApi, proxy)
        {
            if (useSandbox)
            {
                apiUrl = CoinbaseConstants.TestApiUrl;
                apiCheckoutUrl = CoinbaseConstants.TestCheckoutUrl;
            }
        }
        ...
        ///     https://developers.coinbase.com/api/v2#api-key
        /// </param>
        public CoinbaseApi(string apiKey, string apiSecret, string apiUrl = CoinbaseConstants.LiveApiUrl, string checkoutUrl = CoinbaseConstants.LiveCheckoutUrl, bool useTimeApi = true, WebProxy proxy = null)
        {
            ...
            if (string.IsNullOrWhiteSpace(apiKey))
                throw new ArgumentException("The API key must be specified.", nameof(apiKey));
            if (string.IsNullOrWhiteSpace(apiSecret))
                throw new ArgumentException("The API secret must be specified.", nameof(apiSecret));

            this.apiKey = apiKey;
            this.apiSecret = apiSecret;

            this.apiUrl = !string.IsNullOrWhiteSpace(apiUrl) ? apiUrl : CoinbaseConstants.LiveApiUrl;
            apiCheckoutUrl = !string.IsNullOrWhiteSpace(checkoutUrl) ? checkoutUrl : CoinbaseConstants.LiveCheckoutUrl;
        ...
        protected virtual IAuthenticator GetAuthenticator() => new CoinbaseApiAuthenticator(apiKey, apiSecret, useTimeApi, JsonSettings);
        ...
        public virtual CoinbaseResponse<TResponse> SendGetRequest<TResponse>(string endpoint, params KeyValuePair<string, string>[] queryParams)
        {
            ...
            return client.Execute<CoinbaseResponse<TResponse>>(req).Data;
        }
        ...
        ///     Using this endpoint to create checkouts and orders is useful when you want to build a merchant checkout experience
        ///     with Coinbase’s merchant tools.
        /// </summary>
        public virtual CoinbaseResponse CreateCheckout(CheckoutRequest checkout) => SendRequest("checkouts", checkout);
        ...
        /// <returns></returns>
        public virtual Notification GetNotification(string json) => JsonConvert.DeserializeObject<Notification>(json, JsonSettings);
    }
}