...
namespace Coinbase
{
    public class CoinbaseApiAuthenticator : IAuthenticator
    {
        ...
        public static string GenerateSignature(string timestamp, string method, string url, string body, string appSecret) => GetHMACInHex(appSecret, timestamp + method + url + body);
        public static long GetCurrentUnixTimestampMillis() => (long)(DateTime.UtcNow - UnixEpoch).TotalMilliseconds;
        public static DateTime DateTimeFromUnixTimestampMillis(long millis) => UnixEpoch.AddMilliseconds(millis);
        public static long GetCurrentUnixTimestampSeconds() => (long)(DateTime.UtcNow - UnixEpoch).TotalSeconds;
        public static DateTime DateTimeFromUnixTimestampSeconds(long seconds) => UnixEpoch.AddSeconds(seconds);
    }
}